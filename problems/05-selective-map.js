...
let selectiveMap = function (array, cb1, cb2) {
    let newArr = []; // made new array.
    for (let i = 0; i < array.length; i++) { // ittrate throught the array for length
        let ele = array[i]; // set varible for each index
        if (cb1(ele) === true) { // if the ele passed through cb1 is true.
            newArr.push(cb2(ele)); // push the value of the ele passed through the cb2 to new array.
        } else { // if not true,
            newArr.push(ele); // only push ele.
        }
    }
    return newArr;
};
